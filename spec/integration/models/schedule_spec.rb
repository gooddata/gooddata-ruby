...
describe GoodData::Schedule, :vcr, :vcr_all_cassette => 'schedule_model_all' do
  ...
  describe '#create' do
    ...
    it 'Throws exception when no executable specified' do
      expect do
        GoodData::Schedule.create(PROCESS_ID, @test_cron, nil, @test_data)
  ...
  describe '#hidden_params' do
    ...
    it 'Should return hidden_params as hash. Filled by default if provided.' do
      schedule = GoodData::Schedule.create(PROCESS_ID, @test_cron, @project_executable, @test_data.merge(hidden_params: { 'a' => 'b' }))
      res = schedule.hidden_params
      expect(res).not_to be_nil
      expect(res).to be_a_kind_of(Hash)
  ...
  describe '#update_params' do
    it 'Updates the params and marks the object dirty' do
      schedule = GoodData::Schedule.create(PROCESS_ID, @test_cron, @project_executable, @test_data.merge(params: { 'a' => 'b' }))
  ...
  describe '#state' do
    ...
    it 'Should return schedule state as string as provided' do
      schedule = GoodData::Schedule.create(PROCESS_ID, @test_cron, @project_executable, @test_data.merge(:state => 'DISABLED'))
      res = schedule.state
      expect(res).not_to be_nil
  ...
  describe '#name' do
    ...
    it 'should be able to return your name if specified during creation.' do
      schedule = GoodData::Schedule.create(PROCESS_ID, @test_cron, @project_executable, @test_data_with_optional_param.merge(name: 'My schedule name'))
