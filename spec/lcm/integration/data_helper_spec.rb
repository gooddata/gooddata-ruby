...
postgresql_basic_params = {
    ...
    "input_source"=> {
        "type"=> "postgresql",
        "query"=> "SELECT DISTINCT client_id, segment_id, project_title FROM clients"
    }
}
...
describe 'data helper', :vcr do
  ...
  it 'connect to postgres with BASIC authentication' do
    ...
    puts "DEBUG tuqt: postgres: #{file_path}"
    data = File.open('spec/data/postgresql_data.csv').read
    puts "DEBUG tuqt: data read file:#{data}"
    pust "DEBUG tuqt: data file_path:#{file_path}"
    expect(data).to eq File.open(file_path).read
  end
end
