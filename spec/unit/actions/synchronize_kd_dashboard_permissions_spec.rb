...
require 'gooddata/lcm/actions/synchronize_kd_dashboard_permission'

describe GoodData::LCM2::SynchronizeKDDashboardPermissions do
  ...
  before do
    ...
    kd_dashboards = [{
        'projectDashboard' => {
            ...
            'meta' => {
                'tags' => '',
                'summary' => '',
                'title' => 'KD Dashboard 1',
                'sharedWithSomeone' => 1
            }
        }
    }]
    allow(GoodData::AnalyticalDashboard).to receive(:get).and_return(kd_dashboards)

    kd_dashboard_grantees = {
        granteeURIs: {
            items: [
    ...
    allow(GoodData::Mixin::MdGrantees).to receive(:grantees).and_return(kd_dashboard_grantees)
    ...
    allow(dashboard).to receive(:title).and_return('KD Dashboard 1')
    allow(from_project).to receive(:analytical_dashboards).and_return([dashboard])
    allow(from_project).to receive(:user_groups).and_return(user_groups)
    allow(to_project).to receive(:analytical_dashboards).and_return([dashboard])
    allow(to_project).to receive(:user_groups).and_return(user_groups)
  end

  it 'should transfer KD dashboard permission' do
    result = [{ from_project_name: 'src title', from_project_pid: 'to_project_id', status: 'ok' }]
