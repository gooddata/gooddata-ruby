...
This library is able to load data but it is not used that much. Since there is some data processing needed we rely on faster implementations in Java usually. Let us know if you would be interested
...
Sometimes it is useful to compute reports outside of UI so there are couple of convenience methods for that.
	...
	report = GoodData::Project[1231]
	result = report.execute
	pp result

	File.open('png.png', 'w') do |f|
		f.write(rep.export(:png))
	end
