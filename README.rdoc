...
This is a two-in one tool. It both contains a wrapper class called Gooddata::Client
...
To install the gooddata gem, you instead have to first checkout this repository:

  git clone git://github.com/gooddata/gooddata-ruby.git

Then install the gem using the rake task:
...
As you saw if you ran the above test command <tt>gooddata</tt> will prompt you for your GoodData username
...
/ <tt>-u</tt> and <tt>--password</tt> / <tt>-p</tt>. Examples:
  ...
  $ gooddata -u user@example.com
  Password: 

  $ gooddata -u user@example.com -p secret
...
If you don't wish to write your credentials each time you connect to GoodData using <tt>gooddata</tt>, you can
create a simple gooddata settings file called <tt>.gooddata</tt> in the root of your users home directory. To make it
easy you can just run the settings file generator command which will create the file for you:

  gooddata --write-settings
