  ...
  class BlobStorageClient
    ...
    def initialize(options = {})
      ...
      if options['blobStorage_client']['connectionString'] || options['blobStorage_client']['container']
        @connectionString = options['blobStorage_client']['connectionString']
        @container = options['blobStorage_client']['container']
        @path = options['blobStorage_client']['path']
        @use_sas = false
        build_SAS(@connectionString)
      else
    ...
    def connect
      ...
      if use_sas
        ...
        @client = Azure::Storage::Blob::BlobService.create_from_connection_string(@connectionString)
      end
    end
